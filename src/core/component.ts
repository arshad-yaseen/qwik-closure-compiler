...
import { type FunctionNode, is, isFunctionExpr, isHostTag, tagName, unwrap } from "./ast";
...
export function resolveElement(module: Module, opening: JSXOpeningElement): Component | "host" | null {
...
export function findPropBinding(component: Component, prop: string): PropBinding | Failure {
  ...
  if (is(param, "ObjectPattern")) {
    for (const property of param.properties) {
      ...
      if (!is(property.value, "Identifier")) return { fail: `unsupported pattern for prop '${prop}'` };
